										...
										<div class="tab-content">
											<div id="kt_project_users_card_pane" class="tab-pane fade show active">
												<div class="row g-6 g-xl-9 data">
													@include('receiver.common.jobs-data')
...
<script type="text/javascript">
		  ...
		  $(document).on('click','.apply_btn',function(){
			  var id = $(this).attr('id');
			  var url = '{{ route("receiver.apply_job", ":id") }}';
			  ...
			  $.ajax({
				  ...
				  success:function(res){
					if(res == 1){
						...
						$('#'+id).removeClass('apply_btn btn-success').
						addClass('remove_apply btn-default').html('Cancel');
					}
				  }
			  })
		  })
		  ...
		  $(document).on('click','.remove_apply',function(){
			  var id = $(this).attr('id');
			  var url = '{{ route("receiver.remove_apply", ":id") }}';
			  url = url.replace(':id', id);
			  $.ajax({
				  url:url,
				  type:"GET",
				  success:function(res){
					if(res == 1){
						Swal.fire(
 						 '',
  						'Successfully Removed From Hiring!',
  						'success'
						);
						$('#'+id).removeClass('remove_apply btn-default').
						addClass('apply_btn btn-success').html('Hire');
					}
				  }
			  })
		  })
